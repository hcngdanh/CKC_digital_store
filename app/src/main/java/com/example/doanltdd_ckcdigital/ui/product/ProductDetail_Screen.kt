            ...
            Column(
                ...
                Column(Modifier.fillMaxWidth().background(Color.White).padding(16.dp)) {
                    ...
                    if (categoryId in listOf(1, 6, 7, 2, 8, 9, 10)) {
                        SpecRow("Ngàm ống kính", product!!.LensMount)
                    ...
                    SpecRow("Chi tiết", product!!.Resolution)
                    HorizontalDivider(color = Color(0xFFEEEEEE))
                    SpecRow("Bảo hành", product!!.WarrantyPeriod)
                }
...
fun ReviewItem(review: Review) {
    Column(Modifier.fillMaxWidth().padding(vertical = 8.dp)) {
        ...
        val dateDisplay = try { review.ReviewDate.split("T")[0] } catch(e: Exception) { review.ReviewDate }
        Text(dateDisplay, fontSize = 11.sp, color = Color.Gray)
